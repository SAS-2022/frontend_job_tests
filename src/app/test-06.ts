...
@Component({
    ...
    template : `
                <h2>User Review:</h2>
                <textarea class="textfield" placeholder="Write your Review" [value]="review_input" (keyup) = "showText($event.target.value)"></textarea>
                <br/><br/>
                <h3>Output:</h3>
...
At https://wallethub.com <b>bolded text</b>`
    ...
    showText(text: string) {
        if(text != null) {
            ...
            let urlMatch = text.match(/(\b(https?|ftp|file|www?):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig);
            this.review_content = text;
            if(urlMatch != null) {
                urlMatch.map(url => {
                    this.review_content.replace(url, "<a href>" + url + "</a>");
                    console.log('the url: ', this.review_content);
                }) 
            } 
        }
